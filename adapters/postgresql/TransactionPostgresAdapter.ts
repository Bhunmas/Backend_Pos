...
//join employees e on ${value.Employee_id} = e.employee_id
            ...
            const res = await this.client.query(`insert into transactions(product_id,employee_id,product_name,quantity,total_price,employee_name) select ${value.Product_id},${value.Employee_id},p.product_name,${value.Quantity},${value.Total_price},e.employee_name from Transactions t join products p on ${value.Product_id} = p.product_id
join employees e on ${value.Employee_id} = e.employee_id `);
            resolve(res.rows);
            connect.release();
        })
